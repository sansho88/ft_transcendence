...
import { OriginContext, UserContext, LoggedContext } from "@/context/GameContext";
...
export default function WebsocketClient({className, classNameBlockMessage}: {className: string, classNameBlockMessage: string}) {
	...
	const [chatMsgs, setChatMsgs] = useState<IChatMessage[]>([]);
	...
	const connectToWebsocket = () => {
		...
		if (socketRef.current) {
			...
			socketRef.current.on("getallmsgObj", (messages: IChatMessage[]) => {
			...
			socketRef.current.on("responseObj", (obj: IChatMessage) => {
				console.log(
